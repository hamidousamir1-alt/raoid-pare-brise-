...
CREATE TABLE IF NOT EXISTS prospect_notes(id uuid PRIMARY KEY DEFAULT gen_random_uuid(),prospect_id uuid NOT NULL REFERENCES prospects(id) ON DELETE RESTRICT,note_type text NOT NULL DEFAULT 'note' CHECK(note_type IN ('note','qualification','need','objection','meeting_note')),content text NOT NULL,next_action text,next_action_at timestamptz,created_at timestamptz NOT NULL DEFAULT now(),updated_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX IF NOT EXISTS prospect_notes_prospect_idx ON prospect_notes(prospect_id,created_at DESC);
