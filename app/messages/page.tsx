              ...
              {data.campaignRuns.length ? data.campaignRuns.map((run) => (
                <article className="campaignRun" key={run.id}>
                  ...
                  <div className="campaignImpact">
                    <small>IMPACT OBSERVÉ SOUS 90 JOURS</small>
                    <b>{run.appointments} RDV · {run.offers} offre(s) · {run.won} partenaire(s) gagné(s) · {new Intl.NumberFormat("fr-FR", { style: "currency", currency: "EUR", maximumFractionDigits: 0 }).format(run.generatedRevenue)}</b>
                  </div>
