...
type Sequence = { id: string; name: string; description: string; active: boolean; stopOnReply: boolean; steps: number };
type Message = {
  id: string;
  prospect: string;
  recipient: string;
        ...
        <>
          ...
          <section className="mailGrid">
            <div className="mailPanel">
              ...
              <hr style={{border:0,borderTop:"1px solid #edf0f4",margin:"22px 0"}} />
              <h2>Lancer une séquence</h2>
              <div className="mailForm">
                <label>
                  Parcours automatique
                  <select value={sequenceId} onChange={(e)=>setSequenceId(e.target.value)}>
                    {data.sequences.map(s=><option value={s.id} key={s.id}>{s.name} — {s.steps} étapes</option>)}
                  </select>
                </label>
                <p className="muted">Les relances s’arrêtent automatiquement dès qu’une réponse est détectée.</p>
                <button type="button" className="primary" disabled={busy||!prospectId||!sequenceId} onClick={enroll}>Démarrer la séquence</button>
              </div>
            </div>
            <div className="mailPanel">
