  ...
  try {
    const [funnel, activity, revenue, emails, calls, rankings, risks, goals] =
      await Promise.all([
        ...
        db()`select count(*) filter(where direction='outbound' and status in ('sent','delivered','replied'))::int as sent,count(*) filter(where direction='inbound')::int as replies,count(*) filter(where direction='inbound' and reply_category in('interested','callback','information'))::int as "positiveReplies",(select count(*) from email_enrollments where status='active')::int as "activeCampaigns" from email_messages where created_at>=date_trunc('month',now())`,
